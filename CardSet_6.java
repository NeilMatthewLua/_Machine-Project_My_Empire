...
/**
 * Card class which acts as a template for the cards that can be used during the
 * game
 *
...
public class CardSet_6 extends Card {
    ...
    public String useCard(Player player, GameBoard gameBoard) {
        String event = "";
        int nIndex = player.getCards().get(player.getCards().size() - 1).getIndex(); // gets the index of the card to
                                                                                     // navigate through its set
        ...
        if (player.giveMoney(gameBoard.getBank(), arrChoices[value])) {// If player can't pay then he gives all he has
                                                                       // and loses
        ...
        gameBoard.addCardDiscard(player.getCards().get(player.getCards().size() - 1)); // put used card into card
                                                                                       // discarded pile
